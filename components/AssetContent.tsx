...
const AssetContent: React.FC<Props> = ({ assets }) => {
  const currency = "USD";
  return (
    <div className="flex flex-col gap-5 text-[#FAFAFA] mx-32">
            ...
            {assets &&
              assets.map((asset, key) => (
                <tr key={key} className="py-2 items-center py-1">
                  ...
                  <td className={`font-medium text-lg ${asset.change_24h_percent < 0 ? 'text-red-500' :
                      asset.change_24h_percent > 0 ? 'text-cyan-500' :
                        'text-gray-500'
                    }`}>
                    {asset.change_24h_percent}%
                  </td>
                  <td className={`font-medium text-lg ${asset.change_24h_value < 0 ? 'text-red-500' :
                      asset.change_24h_value > 0 ? 'text-cyan-500' :
                        'text-gray-500'
                    }`}>
